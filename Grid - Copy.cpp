...
//packet << 110 << _ints[0] << _ints[1] <<  _ints[2];
        ...
        for ( int i = 0; i < _ints.size(); i++ )
        {
            ...
            deckout.push_back( stack.cubeUnedited[ _ints[ i ] ] );
...
///std::cout << _ints[0] << _ints[1] <<  _ints[2]  << std::endl;
    ...
    for ( int i = 0; i < _ints.size(); i++ )
    {
        ...
        alldecks[ stack.whosTurn ].push_back( stack.cubeUnedited[ _ints[ i ] ] );
    }
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        ///Swap the orig;
        std::swap( stack.cubeUnedited[ _ints[ i ] ], stack.cubeUnedited[ gp.current ] );
        gp.current++;
...
void Grid::Loop( )
{
    ...
    */
    ...
    std::vector< std::string > cube_no_set = cr( cube );
    /// Get setname...
